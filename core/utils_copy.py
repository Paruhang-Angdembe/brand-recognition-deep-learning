...
def image_preprocess(image, target_size):
    ...
    # Resize and pad image to match the target size
    ih, iw = target_size
    h, w, _ = image.shape
    scale = min(iw/w, ih/h)
    ...
    image_paded /= 255.0  # Normalize to [0, 1]

    # Convert image to PyTorch tensor and rearrange to C, H, W
...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        print("Drawing bbox: ", coor)  # Diagnostic print
...
def nms(bboxes, iou_threshold, sigma=0.3, method='nms'):
    ...
    """
    # Check if bboxes is empty or not a 2-dimensional array
    if not isinstance(bboxes, np.ndarray) or bboxes.ndim != 2 or bboxes.size == 0:
        return []
